...
import { useUpdateKeluarga } from "@/networks/keluarga";
...
interface ModalEditKeluargaProps {
  isOpen: boolean;
  onClose: () => void;
  data: KeluargaProps;
}
...
const ModalEditKeluarga = ({
  isOpen,
  onClose,
  data,
}: ModalEditKeluargaProps) => {
  ...
  useEffect(() => {
    if (isOpen && data) {
      form.reset({
        ...
        anggota_ids: data.anggota.map((a) => a.id),
      });
    }
  }, [isOpen, data, form]);
...
export default ModalEditKeluarga;
